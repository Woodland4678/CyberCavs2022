...
rev::SparkMaxPIDController tempShooterPidController = followMotor.GetPIDController();
rev::SparkMaxRelativeEncoder shooterEncoder = leaderMotor.GetEncoder();
rev::SparkMaxRelativeEncoder tempShooterEncoder = followMotor.GetEncoder();
...
//AddChild("hoodSolenoid", hoodSolenoid);
    ...
    //****************************************/// Put this follow line back in when done with 2 wheeled shooter prototype
    ...
    //temp code
    tempShooterPidController.SetP(0.0003);
    tempShooterPidController.SetI(0.000001);
    tempShooterPidController.SetD(0);
    tempShooterPidController.SetFF(0);
    tempShooterPidController.SetOutputRange(-1, 1);
}
...
void Shooter::TempSetLowerShooterRPM(double rpm1) {
    shooterPidController.SetReference(rpm1, rev::ControlType::kVelocity);
    //leaderMotor.Set(0.5);
}
void Shooter::TempSetHigherShooterRPM(double rpm2) {
    //followMotor.Set(0.5);
    tempShooterPidController.SetReference(rpm2, rev::ControlType::kVelocity);
}
double Shooter::TempGetHigherMotorActualRPM() {
    return shooterEncoder.GetVelocity();
}
double Shooter::TempGetLowerMotorActualRPM() {
    return tempShooterEncoder.GetVelocity();
}
