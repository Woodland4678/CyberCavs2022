...
// fpt2 = fopen("/media/sda1/test2.txt","w"); // Open file for logging.
...
void Intake::Periodic() {
    // fprintf(fpt2,"intake %d, %d, %d, %d\n", isReversingIntake, GetLowSensor(), ballCount, ballCounterState);
    if (!isReversingIntake){    
        ...
        switch (ballCounterState) {
            case 0:
                if (GetLowSensor() == false) {
                    // fprintf(fpt2,"Ball increased!! was %d ",ballCount);
                    ballCount++;
                    ballCounterState++;
                    // fprintf(fpt2,"now %d \n",ballCount);
                    if (ballCount > 2) {
...
void Intake::ResetBallCounterState() {
    ballCounterState = 0;
}
void Intake::ResetBallShotState() {
    ballShotState = 0;
}
