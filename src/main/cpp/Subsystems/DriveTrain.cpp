...
// Factor needed to convert encoder readings to cm.
// FACTORCM is for low gear
// FACTORHSCM is for high gear.
#define FACTORCM 0.381127380342979
#define FACTORHSCM 0.167696047350911
...
leftDriveLeaderMotor.SetSmartCurrentLimit(50);
rightDriveLeaderMotor.SetSmartCurrentLimit(50);
leftDriveFollowMotor.SetSmartCurrentLimit(50);
rightDriveFollowMotor.SetSmartCurrentLimit(50);
...
location.locx = -230; // position in cm
location.locy = -65; // position in cm
location.gyroOffset = 1.5; // Starting at 1.5deg heading downwards which would be +1.5 degrees.
location.leftEncoder = getRightEncoder(); // / FACTORCM; // Init encoder deltas
location.rightEncoder = getLeftEncoder(); //  / FACTORCM; // both encoders.
location.inLowSpeed = true; // By default, robot is in low gear.
...
void DriveTrain::Periodic() {
    ...
    rightPos = getRightEncoder(); // / 0.02183; // decreases when moving forward so use -ve value.
    leftPos = getLeftEncoder();   //  / 0.02183; // increases when moving forward (value in mm)
    factor = (location.inLowSpeed)?FACTORCM:FACTORHSCM; // Determine the scale factor based on the shifter position.
    frc::SmartDashboard::PutNumber("inLowSpeed",location.inLowSpeed);
    rightDiff = (location.rightEncoder - rightPos)/factor; // Get change in right wheel.
    leftDiff = (location.leftEncoder - leftPos)/factor; // get change in left wheel.
    // We'll use the direction that is mid way between current heading and previous heading.
    curHeading = getGyroReading() + location.gyroOffset; // Ajust by gyroOffset.
    if (curHeading < 0) // Keep in range 0..360
        curHeading += 360.0;
    if (curHeading >= 360.0)
        curHeading -= 360.0;
    if (fabs(curHeading - location.heading)>180) // in the event that we're near the 0/360 cross over point, 
        {
        if (curHeading < 180) // curHeading is likely a value close to 0.0
            {
            heading2Use = (curHeading + (location.heading - 360.0)) / 2; // make sure location.heading is near 0.0
            }
        else // curHeading is likely close to 360.0
            {
            heading2Use = (curHeading + (location.heading + 360.0)) / 2; // make sure location.heading is near 360.0
            }
        }
    else // should be fine.  We're not in the 360 cross over area.  Average the 2 headings.
        heading2Use = (curHeading + location.heading) / 2;

    if (heading2Use < 0) // Keep in range 0..360
        heading2Use += 360.0;
    if (heading2Use >= 360.0)
        heading2Use -= 360.0;

    avgdist = (rightDiff + leftDiff) / 2.0; // Get average distance in cm 

    // Adjust the x,y position of the robot.
    location.locx -= avgdist * std::cos(heading2Use * M_PI / 180.0);
    location.locy += avgdist * std::sin(heading2Use * M_PI / 180.0);

    location.rightEncoder = rightPos; // record current position and direction for next time.
    location.leftEncoder = leftPos;
    location.heading = curHeading;

    frc::SmartDashboard::PutNumber("Location x ", (location.locx));
    frc::SmartDashboard::PutNumber("Location y ", (location.locy));
    frc::SmartDashboard::PutNumber("Location Heading ", (location.heading));

    // Pressure sensor is a high quality P51-200-G-A-I36-4.5V.  It measures 0.5V at 0 PSI and 4.5V at 200.0 PSI
    // 
    pressure = (pressureSensor->GetAverageVoltage() - 0.5) * 200.0 / 4.0; // Do the math to get an actual pressure reading in PSI.
    frc::SmartDashboard::PutNumber("Tank Pressure",pressure);
...
double DriveTrain::getTankPressure(){
    return pressure;
}
